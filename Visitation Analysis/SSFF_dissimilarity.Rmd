...
#droplevels(ssff_data[ssff_data$Year != 2001 & ssff_data$Year != 2018,])
...
plot(residuals(mod.outlierdrop))

testing<-lm(Species~Sex, data =data.outlierdrop)

residuals(randomESNmod)

plot(residuals(randomESNmod), xaxt = 'n' )
axis(side = 1, at=1:40)

ESN$residuals<-residuals(randomESNmod)
data.outlierdrop$residuals<-residuals(mod.outlierdrop)

ggplot(ESN, aes(Year, residuals))+geom_point(aes(fill = Sex), shape = 21, size = 3)+theme_bw()+ggtitle("Full")
ggplot(data.outlierdrop, aes(Year, residuals))+geom_point(aes(fill = Sex), shape = 21, size = 3)+theme_bw()+ggtitle("outlierdrop")

acf(residuals(randomESNmod))


durbinWatsonTest(testing, simulate = TRUE, max.lag = 15)
...
testingoverlap<-lm(Species~Sex, data = ESNoverlap)
...
?acf
durbinWatsonTest(testingoverlap, max.lag = 5)
acf(residuals(randomESNmodoverlap))
plot(residuals(randomESNmodoverlap))

glsmod.esn<-gls(Species~Sex, correlation = corARMA(p=5), data = ESN)

summary(glsmod.esn)
acf(residuals(glsmod.esn))
plot(residuals(glsmod.esn))
